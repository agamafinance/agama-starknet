...
- **strk20-integration/**: the reproducible STRK20 sources. `demo-shielded.ts` runs the shielded
  ...
  Lend to agUSD / Redeem) backed by the real privacy pool + mock prover on a local devnet, served
  on port 3012 via launchd.
...
- On-chain invariants verified live: `NAV == idle + Σ pool.value`, `vault USDC == realized`,
  pools hold no USDC (marking-only), `NAV >= realized`.
- STRK20: 313 real StarkWare + Agama contract tests, plus the local devnet shielded e2e.
